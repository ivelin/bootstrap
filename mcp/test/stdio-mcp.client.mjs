...
async function main() {
  ...
  try {
    ...
    assert.equal(info.osVersion, "2.8.11");
    ...
    assert.match(JSON.stringify(info.houseRules), /legal paper cannot promote/i);
    assert.match(JSON.stringify(info.houseRules), /advisor ride-along is assumed, not observed/i);
