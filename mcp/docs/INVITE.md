...
1. Identity + fail-closed + invite migrations, in order: `20260829_bootstrap_mcp_identity.sql`, `20260909_bootstrap_mcp_fail_closed_invite.sql`, `20260910_bootstrap_mcp_invite_accept.sql`, then the label-qualify fix `20260910_bootstrap_mcp_invite_qualify_label.sql`, then the pgcrypto search_path fix `20260911_bootstrap_mcp_invite_pgcrypto_search_path.sql` (CREATE OR REPLACE `bootstrap_mcp_invite_member` — shipped accept / qualify files are not edited).
...
PGlite role-play: [`E2E_ROLEPLAY.md`](E2E_ROLEPLAY.md) · `mcp/test/e2e-roleplay-matrix.test.mjs` · `mcp/test/invite.test.mjs` (unset store vs RPC status/body) · `mcp/test/identity-pglite.test.mjs` (`SELECT bootstrap_mcp_invite_member` — no 42702 / no 42883).
