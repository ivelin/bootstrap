...
DECLARE
  cid uuid;
  slug text := lower(trim(coalesce(p_idea, '')));
  label text := nullif(trim(coalesce(p_name, '')), '');
...
BEGIN
  ...
  END IF;
  IF slug IS NULL OR slug = '' OR slug !~ '^[a-z0-9][a-z0-9_-]{0,31}$' THEN
    RETURN jsonb_build_object('ok', false, 'error', 'invalid idea slug');
  END IF;
  cid := public.bootstrap_os_ensure_company(p_company);
  IF EXISTS (SELECT 1 FROM bootstrap_os.ideas i WHERE i.company_id = cid AND i.slug = slug) THEN
  ...
  PERFORM bootstrap_os.emit_audit(
    cid,
    (SELECT id FROM bootstrap_os.ideas WHERE company_id = cid AND slug = slug),
    coalesce(email, 'unknown'),
    'mcp',
    jsonb_build_object('op', 'create_idea', 'idea', slug, 'why', coalesce(nullif(p_why, ''), 'new 0-1 board'))
  );
  RETURN public.bootstrap_os_get_journey(p_company, slug);
END;
$$;
