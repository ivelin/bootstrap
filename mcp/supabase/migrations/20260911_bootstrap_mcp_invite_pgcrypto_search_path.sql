-- Restore pgcrypto visibility on invite_member token generation.
...
-- Shipped 20260910_bootstrap_mcp_invite_accept.sql and
-- 20260910_bootstrap_mcp_invite_qualify_label.sql are left in place.
--
-- Root cause (42883): bootstrap_mcp_invite_member SET search_path = public only.
-- That strips the Supabase extensions schema where pgcrypto’s gen_random_bytes lives.
-- Session search_path includes extensions; the SECURITY DEFINER body does not.
-- pgcrypto is already installed (CREATE EXTENSION IF NOT EXISTS is a no-op if present).
-- Match public.bootstrap_mcp_hash_token: SET search_path = public, extensions.
-- Also qualify extensions.gen_random_bytes so a later search_path trim cannot regress.
--
-- Audit: bootstrap_mcp_accept_invite does not call gen_random_bytes / gen_random_uuid
-- in the function body (table DEFAULT gen_random_uuid is pg_catalog on PG13+).
-- Leftover bootstrap_mcp_mint_token has the same public-only + gen_random_bytes
-- pattern; it is not the mentee invite path and is not rewritten here.
