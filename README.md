...
The hosted slice is read-only: OS info, docs, house-rule pins (no login). Optional whoami + company labels after pirin.ai login on `/bootstrap-os/login` (Web Builder; not this repo). Markdown on GitHub remains the constitution. Founder `company-state` stays on path 3 local stdio — not on a shared server.
