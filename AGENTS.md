...
3. When helping a founder adopt the OS, present the Day-0 order from [README.md](README.md#how-to-use-this-pick-one):
   ...
   - **Path 3 — Optional local MCP** under [`mcp/`](mcp/) when they run several ideas — one connector, isolated instances. Do not require MCP or install. Hosted MCP does not exist; keep future-only.
...
6. Honor OS 2.8.5 in any MCP guidance: stated / synthetic / observed (observed wins); spoken yes cannot promote; do not seed from a demographic one-liner (demo-only role-play is the weak case); no Likert / naked dollar WTP (choice or sentence, then map); several ideas allowed (rank and kill per board).
...
10. **Local MCP multi-company:** one connector under `mcp/`; instances isolated by companyId under `BOOTSTRAP_DATA_ROOT`. Never merge control planes across ideas. Hosted remains future-only.
