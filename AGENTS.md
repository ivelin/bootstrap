...
3. When helping a founder adopt the OS, present the Day-0 order from [README.md](README.md#how-to-use-this-pick-one):
   ...
   - **Path 4 — Preview only.** [`plugin/`](plugin/) + HTTP read adapter. Production pin `https://mcp.bootstrap.pirin.ai/mcp`. Git-branch previews on `*.vercel.app`. Not mentee-ready boards. No public catalog submit (team Import from Repo only). Not pirin.ai. Path 1 stays the front door.
...
6. Honor OS 2.8.9 in any MCP guidance: stated / synthetic / observed (observed wins); spoken yes cannot promote; do not seed from a demographic one-liner (demo-only role-play is the weak case); no Likert / naked dollar WTP (choice or sentence, then map); several ideas allowed (rank and kill per board); marketing volume cannot promote ([OS section](company-os/operating-system.md#house-rule-marketing-volume-cannot-promote)); there is no optimal price until people have paid and stayed ([OS section](company-os/operating-system.md#house-rule-there-is-no-optimal-price-until-people-have-paid-and-stayed)); do not automate a step that should not exist ([OS section](company-os/operating-system.md#house-rule-do-not-automate-a-step-that-should-not-exist)). Day 0 / Path 1 question (not a house rule): [lifestyle or swinging for the fences](company-os/operating-system.md#day-0-lifestyle-or-swinging-for-the-fences).
...
11. **Founder lock — login on pirin.ai only.** This repo is the hosted MCP **resource server**. Login and OAuth live on pirin.ai (`/bootstrap-os/login`, Web Builder). Gated tools return 401 + `WWW-Authenticate` to pirin.ai metadata. Do not add a login UI or a copy-paste `bos_` mint as the mentee path. PR cloud agents use PGlite (or an isolated branch DB) — they do not migrate, seed, or live-probe the live pirin.ai Supabase or the production pin.
