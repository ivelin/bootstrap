...
11. **Founder lock — login on pirin.ai only.** This repo is the hosted MCP **resource server**. Login and OAuth live on pirin.ai (`/bootstrap-os/login`, Web Builder). Gated tools return 401 + `WWW-Authenticate` to pirin.ai metadata. Do not add a login UI or a copy-paste `bos_` mint as the mentee path.
